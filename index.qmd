...
Though Poisson named it, the LLN has its origins in a posthumous publication by Jacob Bernoulli over a century earlier, and it has continued developing even up to the twentieth century.[^llnh] The exact details of the law has changed over the centuries. For our purposes, those details are largely irrelevant. Instead, we'll be thinking of the LLN like this: given some population mean and some sample mean, the sample mean approaches the population mean as the sample size increases.
...
The `lln.ani` function [takes six arguments](https://github.com/yihui/animation/blob/main/R/lln.ani.R) but has useful defaults for them all. The most crucial arguments are these three:
...
```{r}
#| eval: FALSE
fn_lln <- function(obs, fun, mean, sd){}
```
