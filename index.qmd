...
[Here's my repository](https://github.com/ampurr/gsoc/tree/public). It has my `.qmd` file, if you'd prefer to look at that or knit it yourself. Unless you also use the accompanying CSS file, there may be slight aesthetic differences between your rendered HTML file and this website.
...
Of course, it's immediately obvious without jitter. In the following heatmap, we can see both the discreteness and distribution of the data. [Thanks to Toby Dylan Hocking for the suggestion to use a heatmap here!]{.aside} However, generating the heatmap is not as straightforward as generating a scatterplot.

First, we need to count the frequency of each sumscore for both extraversion and conscientiousness. Then we need to concatenate the two personality factors and calculate all possible combinations between them. Conceptually simple, but computationally painful.
...
I also messed up with the heatmap, possibly because I'm unfamiliar with them. [Note to self: do more research beforehand.]{.aside}