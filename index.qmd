...
3. The ability to continuously sample from the aforementioned data set and generate sample means, as well as a way to make it stop; and
...
The second requirement is also straightforward. I can just calculate `data_vector`'s mean---by definition, it's the population mean. [For self-evident reasons, `ani.lln` calls it `mu`. As you can tell from my code, I prefer to trade brevity for extreme obviousness. Hence `population_mean`.]{.aside}
...
# generate df
sample_tbl <- sample_means |> 
  data.frame() |>
  tibble::rowid_to_column(var = "n")
```

The fourth requirement takes our population mean and sample means and constructs a plot that showcases the LLN. Constructing a static plot is straightforward, but constructing an animated plot is somewhat more difficult. By default, `animint2` only plots 
...
static_lln_plot <- sample_tbl |> 
  ggplot() +
  aes(x = n, y = sample_tbl$sample_means) + 
  labs(y = "sample mean") +
...
```{r fun-lln}
fn_lln <- function(data_vector, sampling){
  ...
  # if sampling > data_vector, stop
  is_greater_than(x = sampling,
                y = data_vector,
                severity = "stop")
  ...
  # make tibble of sample means
  sample_tbl <- sample_means |> as_tibble()
}
```
...
Does `fn_lln` work right? Let's see.


### Testing in Pieces

Let's test each requirement piece by piece. First, let's ensure that the validity check is working right. `fn_lln` needs to accept a vector like `d6` (which is numeric, non-empty, and contains no `NA` values).

```{r test-validity-check}
fn_lln <- function(data_vector){
  # make sure dataset is a numeric vector
  validity_check <- data_vector |> 
  assertive::assert_is_numeric()
...
test_that("throws error message when using data_vector = non-numeric, empty, or contains NA", {
  expect_error(
...
Here's what my for loop does. First, it samples a datum and produces the mean, which is just the datum itself. Then it replaces that datum, samples 2 data points, and produces a mean. Then it samples 3 data points, then 4, and so on until the function tells it to stop.
