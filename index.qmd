...
The first requirement is straightforward. The dataset needs to take the form of a numeric vector that contains at least one number. If a user tries to use something else, `fn_lln` should throw up an error. I'll call the vector `data_vector`:
...
The third requirement is more complicated. I need to take multiple samples, each one larger than the other, and calculate sample means from them. Then I need to place those calculations into a tibble to use later on.

```{r req-sample-mean}
#| eval: FALSE
sample_means <- data_vector
