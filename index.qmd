...
The `sampling` argument lets `fn_lln` know both the sample size and how often `data_vector` should be sampled, up to `data_vector`'s length. A for loop does the sampling, and then `sample_tbl` stores the sample means as a tibble.
