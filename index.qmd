...
So let's look at the Central Limit Theorem (CLT). [The CLT `animination` example [can be found here]](https://yihui.org/animation/example/clt-ani/).]{.aside} To continue with the psychology theme from the easy test, I'll construct a CLT function and then showcase it with a hypothetical distribution of IQ scores.


### About the CLT

"The Central Limit Theorem" is a misnomer, because there is no one central limit theorem. The name refers to a variety of mathematical ideas and research programs bound together by history and a kind of family resemblance.^[H. Fischer. [Chapter 1](https://link.springer.com/chapter/10.1007/978-0-387-87857-7_1) in _A History of the Central Limit Theorem_. 2010.] Generally, central limit theorems describe a normal distribution that emerges from a large number of independent and random variables.

```{r fig-normal-curve}
#| label: fig-normal-curve
#| fig-cap: "Data distributed normally."
normal <- ggplot(tibble(x = c(-3,3)), aes(x)) + 
  stat_function(fun = dnorm, geom = "point")
normal
```

For the purposes of this test, the exact details of the CLT are not especially important, so this general outline is enough. 
