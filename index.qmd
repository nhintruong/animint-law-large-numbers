...
As mentioned, I also want useful defaults for `fn_lln`. Let's have `data_vector` load a pseudo-random sample of six-sided dice by default. And let's 
...
```{r fun-data-d6}
d6 <- sample(x = 1:6, size = 5000, replace = TRUE)
```
...
BUG: does not correctly sample when `data_vector$size` < `sampling`. grrrr
