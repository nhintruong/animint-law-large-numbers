...
## Medium Test: IQ and the Central Limit Theorem
...
So let's look at the Central Limit Theorem (CLT). To continue with the psychology theme from the easy test, I'll construct a CLT function and then showcase it with a hypothetical distribution of IQ scores.


### About the CLT

"The Central Limit Theorem" is a bit of a misnomer, because there is no one central limit theorem. The name refers to a variety of mathematical ideas and research programs bound together by history and a kind of family resemblance.^[H. Fischer. [Chapter 1](https://link.springer.com/chapter/10.1007/978-0-387-87857-7_1) in _A History of the Central Limit Theorem_. 2010.] Generally, central limit theorems describe a normal distribution that emerges from a large number of independent and random variables.

```{r normal-curve}
#| column: margin

# example of a normal distribution
normal <- tibble(x = c(-3,3)) |>
  ggplot(aes(x)) +
  stat_function(fun = dnorm, geom = "point")
normal
```

Since the CLT _qua_ theorem is not of interest to me or to the animated demonstration I'll be translating, this general outline is enough. 


### Function Construction

The [CLT `animation` demonstration](https://yihui.org/animation/example/clt-ani/) actually consists of two animations: a density plot and a Shapiro-Wilk normality test. My opinion is that the Shapiro-Wilk normality test is of limited pedagogical value, so I've removed it and replaced it with a QQ plot. [Also, I have a personal distaste for null hypothesis testing.]{.aside}

We begin by [looking at the code for `clt.ani`](https://github.com/yihui/animation/blob/main/R/clt.ani.R). The function takes eight arguments, though not all of them are necessary. Let's begin constructing the ones that are.

```{r}
#| eval: FALSE
fn_clt <- function(obs, fun, mean, sd){}
```

First: observations (`obs`). 



First, we need to generate a default seq
