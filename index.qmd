...
## Medium Test: The Central Limit Theorem
...
So let's look at the Central Limit Theorem (CLT). [[Here's the CLT `animination` example](https://yihui.org/animation/example/clt-ani/).]{.aside}


### About the CLT

My name for this test is inaccurate for two separate reasons.

The first is that "the Central Limit Theorem" itself is a misnomer. There is no one single central limit theorem. The name refers to many theorems and mathematical concepts, bound together by history and a kind of family resemblance. The principal connection between them is that they all have something to do with random variables and normal distributions.^[H. Fischer. [Chapter 1](https://link.springer.com/chapter/10.1007/978-0-387-87857-7_1) in _A History of the Central Limit Theorem_. 2010.]

The second is that this test is not really about the central limit theorem as a theorem. What I am actually going to depict is a normal distribution, constructed from psuedo-random data, as described by the CLT.
...
<!-- - When using `geom_hex, I'm warned that "no data in geom1_hex_corrdiscrete." -->
