...
Though Poisson named it, the LLN has its origins in a posthumous publication by Jacob Bernoulli over a century earlier, and it has continued developing even up to the twentieth century.[^llnh] The exact details of the law have changed over the centuries. For our purposes, those details are largely irrelevant. Instead, we'll be thinking of the LLN like this: given some population mean and some sample mean, the sample mean approaches the population mean as the sample size increases.

[^llnh]: M. Denker. [Tercentennial anniversary of Bernoulli’s law of large numbers](doi.org/10.1090/S0273-0979-2013-01411-3). _Bull. Am. Math. Soc_. 2013.
...
1. A function that reads some set of data;
2. A function that generates the population mean for that data;
3. A function that continuously samples from the aforementioned data set and generate sample means, as well as a way to make it stop; and
...
Furthermore, though this isn't necessary for the function to work, I want `fn_lln` to output messages that are useful for someone who's unfamiliar with R. Let's say this criterion applies to all the requirements for `fn_lln`. I'll use the `assertive` package to do this.

The first requirement, which I'll call `distr`, is straightforward. [Short for "distribution," of course.]{.aside} The dataset needs to take the form of a numeric vector. If a user tries to use something else, the `distr` should throw up an error.
...
```{r fun-lln}
fn_lln <- function(distr = d6){
  
}
```

The most crucial arguments are these three:

- `FUN`: A function to generate random numbers.
- `mu`: The population mean.
- `np`: How often the data are sampled.



I'll generate a random sequence of rolls from a six-sided die: [`random` allows for true randomness.]{.aside}

```{r}
# true_randomness <- random::randomNumbers(n = 10000,
#                                          min = 1, max = 6,
#                                          col = 1)
```
...
To aid reproducibility, I'll also save this sequence as a CSV file and read from it later.

```{r}
# random_d6 <- true_randomness |> 
#   as_tibble() |>
#   setNames(c("n")) |> 
#   write_csv(path = "random_d6.csv")
# 
# d6 <- read_csv("random_d6.csv")
```
...
<!-- My sense is that forking `ggplot2` is a double-edged sword. -->
...
<!-- fun fact: the ggplot2 book mentioned animint -->
