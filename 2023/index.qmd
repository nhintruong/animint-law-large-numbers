...
And what about the number of responses per country? [`"(nu"` is equivalent to `NA`.]{.aside}
...
```{r}
six_c <- b5c |> filter(country == c("US", "GB", "IN", "AU", "CA", "PH"))
```
...
```{r}
row_tbl <- rowid_to_column(six_c, "id") |> rowwise(id)
