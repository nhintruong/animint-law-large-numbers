...
What do our data show? Here's an interactive scatterplot---extraversion is plotted against neuroticism.^[Extraversion refers to phenomena like outgoingness, while neuroticism refers to phenomena like emotional stability.]
...
The United States is the only country currently plotted. Clicking on other countries plots their data and shows that aforementioned overlap between countries. Pretty neat. I've also made the data points semi-transparent so the data density is visible.

I haven't bothered to do any statistics. But at first glance, it seems like these data are consistent with the five-factor model. The data seem to trail vaguely upward. Perhaps $r > 0$, but only a little. If we zoom in where the data are most dense, perhaps even that minor pattern will fade.

I think that's a hypothesis worthy of exploration. Let's take a look at one last `animint()` before concluding this part of the easy test.

```{r ani-corr-zoom}
corr_densed <- correlations +
  geom_point() + 
  xlim(20, 40) + 
  ylim(20, 40)
...
This time I've opted to show the Philippines, the country with the sparsest data. The data do seem to be pretty patternless. Furthermore, without jitter, the discrete nature of the data become visable. You can have an extraversion sumscore 

Pretty cool! This kind of exploratory research would be harder without the interactive benefits of `animint2`.
