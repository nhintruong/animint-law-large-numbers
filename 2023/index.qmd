...
library(dplyr)        # tidyverse file manipulate
library(readr)        # tidyverse file reader
library(stringr)      # tidyverse string manipulate
library(tibble)       # tidyverse dfs
...
Lastly, the individual item responses aren't especially interesting to me. [I could use the individual items to estimate a psychometric network. But as far as I can tell, neither `animint2` nor `ggplot2` support that.]{.aside} So I'm going to generate sumscores for each of the five personality factors and use that instead for the interactive visualization. I'll also replace the [ISO 3166]{.smallcaps} codes with the actual country names.
