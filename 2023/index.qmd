...
oc <- sumscores |> 
  ggplot(mapping = aes(x = country, 
                       y = openness))
oc + geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) + 
  expand_limits(y = 100)
```

```{r}
#| eval: FALSE
#| label: fig-violin-personalities
#| fig-cap: "Violin plots of personality factors by country."
#| layout-ncol: 2
#| layout-nrow: 3
#| out-width: 40%
...
cc <- sumscores |> 
  ggplot(mapping = aes(x = country, 
                       y = conscientiousness))
cc + geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) + 
  expand_limits(y = 100)

ec <- sumscores |> 
  ggplot(mapping = aes(x = country, 
                       y = extraversion))
ec + geom_violin(draw_quantiles = c(0.25, 0.5, 0.75))  + 
  expand_limits(y = 100)

ac <- sumscores |> 
  ggplot(mapping = aes(x = country, 
                       y = agreeableness))
ac + geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) + 
  expand_limits(y = 100)

nc <- sumscores |> 
  ggplot(mapping = aes(x = country, 
                       y = neuroticism))
nc + geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) + 
  expand_limits(y = 100)
```
