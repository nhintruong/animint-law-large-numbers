...
correlations <- sumscores |> 
  ggplot(mapping = aes(x = neuroticism,
                       y = extraversion,
                       color = country))
...
Here, extraversion is plotted against neuroticism. [Extraversion refers to phenomena like outgoingness, while neuroticism refers to phenomena like emotional stability.]{.aside} The United States is the only country currently plotted. Clicking on other countries plots their data and shows that aforementioned overlap between countries. I've also made the data points semi-transparent so the data density is visible.

I haven't bothered to do any statistics. But at first glance, it seems like these data are consistent with the five-factor model. The data seem to trail vaguely upward. Perhaps $r \neq 0$, but only a little. If we zoom in where the data are most dense, perhaps even that minor pattern will fade.
...
This time I've opted to show the Philippines, the country with the sparsest data. The data do seem to be pretty patternless. Furthermore, as you add more countries---or just the United States---the discrete nature of the data becomes visible even with jittered points. (For example, it's possible to have an extraversion sumscore of 25 or 26, but nothing in-between.)
...
Mistakes I made while doing these tests (and then subsequently caught). This list is not comprehensive and only records major mistakes.


### Easy Test

My sumscores were erroneous, but it took me an embarassingly long time to notice and check. I first noticed that the openness sumscores were impossibly high---each sumscore has a ceiling of 50, which openness sumscores exceeded. I then 